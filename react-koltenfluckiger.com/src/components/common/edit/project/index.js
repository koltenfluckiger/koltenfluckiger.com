  ...
  render() {
    ...
    return (<div className={styles.subcontainer}>
      ...
      <Form preventEnterSubmit="preventEnterSubmit" id="edit-project-form" handleSubmit={handleSubmit} encType="multiPart">
        ...
        <FormGroup>
          <TextArea variant={{classes: "admin-input"}} name="description" type='text' placeholder='Description' rows="10" required="required" defaultValue={project.description}/>
        </FormGroup>
        <FormGroup>
          <Input variant={{classes: "admin-input"}} required={true} name="sourceCodeLink" autoComplete='off' type='text' placeholder='Source Code Link' defaultValue={project.hostedStatus
              ? "Yes"
              : "No"}/>
        </FormGroup>
        <FormGroup>
          <Input variant={{classes: "admin-input"}} required="required" name="hostedLink" autoComplete='off' type='text' placeholder='Hosted Link' defaultValue={project.hostedLink}/>
          <Input variant={{classes: "admin-input"}} required="required" name="hostedStatus" autoComplete='off' type='text' placeholder="Currently Live" defaultValue={project.hostedStatus}/>
        </FormGroup>
        <FormGroup>
          <Tagger id="project-search-tags" variant={{classes: "tagger-input"}} name="searchTags" placeholder='Search Tags' options={{
              allow_spaces: true,
              onlyWhiteList: false,
              defaultTags: project.searchTags ? project.searchTags : []
            }}/>
        </FormGroup>
        <FormGroup>
        ...
        <FormGroup>
          <Button type="submit" variant={{classes: "button-form-control button green"}} text="Submit Changes"/>
